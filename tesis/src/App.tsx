...
export const App = () => {
  ...
  const { mutate: vertexMutation, isLoading: isVertexPending } = useMutation({
    mutationFn: createVertexQuery.mutation,
    onSuccess: (data) => {
      setResultSqlQuery(data);
    },
  });
  const { mutate: gptMutation, isLoading: isGptPending } = useMutation({
  ...
  return (
    <div className="h-screen w-screen bg-orange-50 flex flex-col items-center">
      ...
      <form
        className="flex flex-col gap-2 justify-center pt-10 w-1/4 items-center"
